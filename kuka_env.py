...
def get_train_env():
    env = gym.make("KukaDiverseObjectGrasping-v0", maxSteps=20, isDiscrete=False, renders=False, removeHeightHack=False)
    env = AssignTypeWrapper(env)
    return env


def get_test_env():
    env = gym.make("KukaDiverseObjectGrasping-v0", maxSteps=20, isDiscrete=False, renders=True, removeHeightHack=False)
    env = AssignTypeWrapper(env)
    return env